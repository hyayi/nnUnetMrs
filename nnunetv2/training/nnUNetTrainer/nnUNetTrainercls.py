...
class nnUNetTrainercls(nnUNetTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.num_epochs = 1300 #1000 #1000#1000
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        return {'loss': l.detach().cpu().numpy(), 'tp_hard': tp_hard, 'fp_hard': fp_hard, 'fn_hard': fn_hard,'cls_output' : cls_output.detach().cpu().numpy(), 'label' : label.detach().cpu().numpy()}
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        cls_out = np.vstack(outputs_collated['cls_output'])
        label = np.hstack(outputs_collated['label'])
        if self.is_ddp:
            world_size = dist.get_world_size()
        ...
        auc = auroc(torch.tensor(cls_out),torch.tensor(label), num_classes=2).item()
        global_dc_per_class = [i for i in [2 * i / (2 * i + j + k) for i, j, k in
                                           zip(tp, fp, fn)]]
        mean_fg_dice = np.nanmean(global_dc_per_class)
        self.logger.log('auroc', auc, self.current_epoch)
    ...
    def on_epoch_end(self):
        ...
        self.print_to_log_file('auroc', np.round(self.logger.my_fantastic_logging['auroc'][-1], decimals=4))
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            for batch_id in range(self.num_iterations_per_epoch):
                ...
                train_outputs.append(self.train_step(data))
